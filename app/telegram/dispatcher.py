...
from app.telegram.types import TelegramMessage, parse_command
...
class TelegramDispatcher:
    ...
    async def handle(self, message: TelegramMessage) -> Optional[str]:
        command = parse_command(message.text)
        if not command.name:
            return None
        if message.actor_id in self._admin_ids:
            return (await self._admin_handler.handle(message, command)).text

        response = await self._user_handler.handle(message, command)
        if response.ok or response.text != "Access denied.":
